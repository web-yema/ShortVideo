				...
				let gesTokenObj = BindingX.bind({
					...
					props: [{
						...
						},
						{
							...
							expression: `${move}!=0 && ${move}*x>0 ?x+${this.distanceX} : ${this.distanceX}`
						},
						{
							element: leftRef,
							property: 'transform.translateX',
							expression: `${move}!=0&& ${move*this.distanceX}<=0&&x+${this.distanceX}<393?x+${this.distanceX} : ${this.distanceX}`
						},
						{
							element: rightRef,
							property: 'transform.translateX',
			...
			let result = BindingX.bind({
				...
				props: [{
					...
					},
					{
						element: swiperRef,
						property: 'transform.translateY',
						expression: `easeOutExpo(t,${this.distance},0,200)`
					},
					{
						element: leftRef,
						property: 'transform.translateX',
